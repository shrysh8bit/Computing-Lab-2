...
def splitDateNews(str_cleaned):
    # print (str_cleaned)
    split_list = str_cleaned.split(",", 1)
    split_list[0] = split_list[0].split(" ")
    split_list[0] = split_list[0][:2]
    if len(split_list) == 0 or len(split_list) == 1:
        return [0,1]
    split_list[0] = split_list[0][0] + " " + split_list[0][1]

    # print("Date", split_list[0])
    # print("News", split_list[1])
    return split_list
...
def dateFormatting(month_fm_suffix, date_in_str):
    ...
    month_fm_suffix_list = month_fm_suffix.split(' ')
...
def parseWorldWideNews():
    ...
    url_suffixes = [['Australia_(2022)', 70, 120]]
    ...
    # ########################## End Of Lexer #########################################
        ...
        def p_news(t):
            '''news : NEWS'''
            # print ("Grammar : ", t[1]) 
            str_clean = cleanReadData(t[1])
            date_news_split = []
            date_news_split = splitDateNews(str_clean)
            # print (date_news_split[1])
            dd_mm_list = date_news_split[0].split(' ')
            # print ("DD MM : ",dd_mm_list[0], dd_mm_list[1])

            if(dd_mm_list[0][0] < '0' or dd_mm_list[0][0] > '9'):
                # print ("in here")
                dd_mm_list[0] = 4
                dd_mm_list[1] = 'January'

            # print (dd_mm_list[0])
            # print ("ds1", dd_mm_list[1])

            suffix = dd_mm_list[1] + " 2022"
            str_date = str(dd_mm_list[0]) + " " + dd_mm_list[1]
            
            int_date_list = dateFormatting(suffix, str_date)
            ...
            news_instance.news = date_news_split[1]
            news_instance.country = 'Australia'
