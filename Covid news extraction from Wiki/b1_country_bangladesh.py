...
def splitInputStr(str, start_index, stop_index):
    # print (f'split fn >{start_index}<  >{stop_index}<')
    count = 0
    for char in str:
        if char == '\n':
            count += 1
...
def parseWorldWideNews():
    ...
    url_suffixes = [['Bangladesh', 158, 430]]
    ...
    # ########################## End Of Lexer #########################################
        ...
        def p_news(t):
            ...
            news_instance.year = 2020
            news_instance.news = news_clean_str
            news_instance.country = 'Bangladesh'
